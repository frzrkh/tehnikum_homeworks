...
class ShopBot
{
    ...
    private $admin = 0; // Ваш id в ТЕЛГРАМ
    private $helloText = "Приветствую \n<b>Чат-Бот-Магазин</b>";
    ...
    private function drawOrder($user_id, $begin)
    {
        ...
        if ($order->rowCount() > 0) {
            ...
            // кнопка
            $buttons[][] = $this->buildInlineKeyBoardButton('Оплатить', 'setPay_0');
    ...
    // Статья 4 - Онлайн - оплата в Телеграмм
     ...
     * @param $data
     */
    private function setPay($data)
    {
        $this->notice($data['id'], "В дальнейшем здесь будет оплата онлайн", true);
    }
